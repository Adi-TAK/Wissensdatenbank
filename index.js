...
app.post('/api/registrieren', async (req, res) => {
  ...
  try {
    db.prepare('INSERT INTO users (name, email, passwort) VALUES (?, ?, ?)').run(name, email, hash);
    res.json({ erfolg: true });
  } catch (err) {
    res.status(400).json({ fehler: 'Email bereits vergeben!' });
  }
});
...
app.post('/api/wissen', (req, res) => {
  ...
  db.prepare('INSERT INTO wissen (titel, inhalt, kategorie, user_id) VALUES (?, ?, ?, ?)')
    .run(titel, inhalt, kategorie, req.session.user.id);
  res.json({ erfolg: true });
});
...
app.delete('/api/wissen/:id', (req, res) => {
  if (!req.session.user) return res.status(401).json({ fehler: 'Bitte einloggen!' });
  db.prepare('DELETE FROM wissen WHERE id = ? AND user_id = ?').run(req.params.id, req.session.user.id);
  res.json({ erfolg: true });
});
...
app.get('/api/admin/users', (req, res) => {
  ...
  const rows = db.prepare('SELECT id, name, email, isAdmin, erstellt_am FROM users').all();
  res.json(rows);
});
...
app.delete('/api/admin/wissen/:id', (req, res) => {
  if (!req.session.user || !req.session.user.isAdmin)
    return res.status(403).json({ fehler: 'Kein Zugriff!' });
  db.prepare('DELETE FROM wissen WHERE id = ?').run(req.params.id);
  res.json({ erfolg: true });
});
